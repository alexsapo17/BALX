...
let MenuScene = new Phaser.Class({
    ...
    preload: function() {
        this.load.image('back_button', 'images/back_button.png'); 

    },
    create: function() {
        this.add.text(470, 150, 'Balls War', { fontFamily: 'Impact', fontSize: 90, fontStyle: 'bold', fill: '#ff0000', stroke: '#ffffff', strokeThickness: 6 });
    
        let buttonBaseStyle = { fontFamily: 'Arial', fontSize: 40, color: '#ffffff'};
        let buttonHoverStyle = { color: '#ff0000' };
    
        let searchMatchButton = this.add.text(515, 350, 'Search Match', buttonBaseStyle)
            .setPadding(16)
            .setStyle({ backgroundColor: '#00ff00' })
            .setInteractive({ useHandCursor: true })
            .on('pointerover', () => searchMatchButton.setStyle(buttonHoverStyle))
            .on('pointerout', () => searchMatchButton.setStyle(buttonBaseStyle))
            .on('pointerup', () => this.scene.start('gameScene'));
    
        let playWithFriendButton = this.add.text(487, 450, 'Play with Friends', buttonBaseStyle)
            .setPadding(16)
            .setStyle({ backgroundColor: '#00ff00' })
            .setInteractive({ useHandCursor: true })
            .on('pointerover', () => playWithFriendButton.setStyle(buttonHoverStyle))
            .on('pointerout', () => playWithFriendButton.setStyle(buttonBaseStyle))
            .on('pointerup', () => this.scene.start('gameScene'));
    
        let backButton = this.add.image(100, 100, 'back_button')
            .setInteractive({ useHandCursor: true })
            .on('pointerover', () => backButton.setTint(0xff0000))
            .on('pointerout', () => backButton.clearTint())
            .on('pointerup', () => window.location.href = 'schermata_iniziale.html');
    }
    });
...
function create() {
    ...
    shootArea.addEventListener('pointerdown', function(event) {
        touchStartPos = { x: event.clientX, y: event.clientY };
    });

    shootArea.addEventListener('pointerup', (event) => {
        touchEndPos = { x: event.clientX, y: event.clientY };
        if (touchStartPos && touchEndPos) {
            let dx = touchEndPos.x - touchStartPos.x;
            let dy = touchEndPos.y - touchStartPos.y;
            let angle = Math.atan2(dy, dx);
            let velocity = { x: Math.cos(angle) * 600, y: Math.sin(angle) * 600 };
            this.bulletManager.fireBullet(player, velocity, isWsOpen, ws, currentMatchId);
            touchStartPos = null;
            touchEndPos = null;
        }
    });
...
function update() {
    ...
    // Nuovo: smorzamento inerziale quando nessuna direzione è premuta
    ...
    if (playerHealth <= 0) {
        ...
        this.add.text(600, 100, 'Hai perso!', { fill: '#fff' });
        endGame();
    } else if (otherPlayerHealth <= 0) {
        // Il giocatore ha vinto
        this.add.text(600, 100, 'Hai vinto!', { fill: '#fff' });
        endGame();
    }
...
function endGame() {
    ws.close(); // Chiudi la connessione WebSocket
...
console.log('Creating other player image at', position.x, position.y);
    ...
    // Aggiungi 10 monete al localStorage
    let playerCoins = localStorage.getItem('playerCoins');
    if (playerCoins === null) {
        playerCoins = 0;
    } else {
        playerCoins = parseInt(playerCoins);
    }
    playerCoins += 10;
